...
@section('content')
                    ...
                    <div class="folders-grid d-flex flex-wrap folder-div" id="folders-grid">
                        @foreach($data as $folder)
                            <div class="folder-card">
                                ...
                                <input type="checkbox" class="folder-checkbox checkbox-style"
    ...
    @php
        ...
        $projectApplication = \App\Models\ProjectApplication::where('user_id', auth()->user()->id)->where('status','Approved')->where('project_id',$project->id)->first();

        if ($projectApplication && $projectApplication->appliedForm){
            $appliedForm = 0;
        }else{
            $appliedForm = 1;
        }
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('#listFoldersTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('user.images.folder.list') }}",
                    data: {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            method: 'post',
                            url: '{{ route("user.images.folder.delete") }}',
                            data: formData,
                            processData: false,
                            contentType: false,
