					...
					<div class="text-center">
						...
						@if ($plan_type == 'subscription')
							<div class="text-center mb-6">
								...
								<p class="fs-14">{{ __('After successful payment, each month you will have') }} <span class="text-info font-weight-bold">{{ number_format($id->characters) }}</span> {{ __('characters and') }} <span class="text-info font-weight-bold">{{ number_format($id->minutes) }}</span>. {{ __('minutes added to your account. To keep your subsciption active in coming month, please provide payments by the end of the current month.') }}.</p>
						...
						@if ($plan_type == 'prepaid')
							<div class="text-center mb-5">
								<p class="mt-5 fs-14">{{ __('You have successfully placed order for ') }} <span class="text-info font-weight-bold">{{ number_format($id->characters) }}</span> {{ __('characters and') }} <span class="text-info font-weight-bold">{{ number_format($id->minutes) }}</span>. {{ __('minutes for your account') }} </p>
								<p class="fs-14">{{ __('Please provide payment to our bank requisites below. Use Order ID number as payment reference') }}.</p>	
								<p class="text-muted fs-12 mb-4">{{ __('Order ID') }}: <span class="font-weight-bold text-primary">{{ $orderID }}</span></p>
								<p class="text-muted fs-12 mb-4">{{ __('Total Payment Due') }}: <span class="font-weight-bold text-primary">{{ number_format((float)$final_price, 2, '.', '') }} {{ config('payment.default_currency') }}</span></p>
								<pre class="text-muted fs-12 mb-4">{{ $bank['bank_requisites'] }}</pre>							
							</div>						
						@endif
