			...
			<div class="card border-0">
				...
				<div class="card-body pt-2">
					...
					<table id='subscriptionPlanTable' class='table' width='100%'>
							<thead>
								<tr>
									<th width="13%">{{ __('Plan Name') }}</th>
									<th width="10%">{{ __('Status') }}</th>
									<th width="10%">{{ __('Price') }}</th>																																																		
									<th width="5%">{{ __('Characters') }}</th>																									
									<th width="5%">{{ __('Minutes') }}</th>																									
									<th width="8%">{{ __('Pricing Plan') }}</th>
									<th width="5%">{{ __('Featured') }}</th>
									<th width="5%">{{ __('Free') }}</th>
									<th width="5%">{{ __('Synthesize') }}</th>
									<th width="8%">{{ __('Voices') }}</th>
	...
	<script type="text/javascript">
		$(function () {
			...
			$(document).on('click', '.deletePlanButton', function(e) {
				...
				Swal.fire({
					title: '{{ __('Confirm Subscription Plan Deletion') }}',
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							success: function (data) {
								if (data == 'success') {
									Swal.fire('{{ __('Subscription Plan Deleted') }}', '{{ __('Subscription plan has been successfully deleted') }}', 'success');	
