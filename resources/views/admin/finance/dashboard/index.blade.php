...
@section('content')						
	<!-- TOP BOX INFO -->
	<div class="row">	
		<div class="col-lg-4 col-md-12 col-sm-12 no-gutters">	
			<div class="col-12">
			...
			<div class="col-12">
				<div class="card overflow-hidden border-0">
					<div class="card-body">
						<div class="d-flex align-items-end justify-content-between">
							<div>
								<p class=" mb-3 fs-12 font-weight-bold">{{ __('Total Minutes Purchased') }} <span class="text-muted">({{ __('Current Month') }})</span></p>
								<h2 class="mb-0"><span class="number-font fs-20">{{ number_format($total_data_monthly['purchased_minutes'][0]['data']) }}</span><span class="ml-2 text-muted fs-11 data-percentage-change"><span class="fs-12" id="purchased_minutes_difference"></span> {{ __('this month') }}</span></h2>
						...
						<div class="d-flex mt-2">
							<div>
								<span class="text-muted fs-12 mr-1">{{ __('Last Month') }}:</span>
								<span class="number-font fs-12"><i class="fa fa-chain mr-1 text-success"></i>{{ number_format($total_data_monthly['purchased_minutes_past'][0]['data']) }}</span>
							</div>
							<div class="ml-auto">
								<span class="text-muted fs-12 mr-1">{{ __('Current Year') }}:</span>
								<span class="number-font fs-12"><i class="fa fa-bookmark mr-1 text-success"></i>{{ number_format($total_data_yearly['total_purchased_minutes'][0]['data']) }}</span>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="col-12">
				<div class="card overflow-hidden border-0">
					<div class="card-body">
						<div class="d-flex align-items-end justify-content-between">
							<div>
								<p class=" mb-3 fs-12 font-weight-bold">{{ __('Total Purchased Minutes Used') }} <span class="text-muted">({{ __('Current Month') }})</span></p>
								<h2 class="mb-0"><span class="number-font fs-20">{{ number_format((float)$total_data_monthly['paid_minutes'][0]['data'] / 60, 2) }}</span><span class="ml-2 text-muted fs-11 data-percentage-change"><span class="fs-12" id="purchased_minutes_used_difference"></span> {{ __('this month') }}</span></h2>
							</div>
							<span class="text-info fs-50 mt-m1"><i class="mdi mdi-account-plus"></i></span>
						</div>
						<div class="d-flex mt-2">
							<div>
								<span class="text-muted fs-12 mr-1">{{ __('Last Month') }}:</span>
								<span class="number-font fs-12"><i class="fa fa-chain mr-1 text-success"></i>{{ number_format((float)$total_data_monthly['paid_minutes_past'][0]['data'] / 60, 2) }}</span>
							</div>
							<div class="ml-auto">
								<span class="text-muted fs-12 mr-1">{{ __('Current Year') }}:</span>
								<span class="number-font fs-12"><i class="fa fa-bookmark mr-1 text-success"></i>{{ number_format((float)$total_data_yearly['total_purchased_minutes_used'][0]['data'] / 60, 2) }}</span>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="col-xl-3 col-lg-6 col-md-6 col-xm-12">
			<div class="card overflow-hidden border-0">
				<div class="card-body">
					...
					<div class="d-flex">
						<div>
							<span class="text-muted fs-12 mr-1">{{ __('Minutes Used') }} ({{ __('Current Year') }}): </span>
							<span class="number-font fs-12">{{ number_format($total_data_yearly['gcp_current_year_minutes'][0]['data']) }}</span>
						</div>
					</div>
				</div>
			</div>
...
@section('js')
	...
	<script type="text/javascript">
		$(function() {
			...
			let spendingVoiceoverData = JSON.parse(`<?php echo $chart_data['total_voiceover_spending']; ?>`);
			let spendingTranscribeData = JSON.parse(`<?php echo $chart_data['total_transcribe_spending']; ?>`);
			let incomeDataset = Object.values(incomeData);
			let spendingVoiceoverDataset = Object.values(spendingVoiceoverData);
			let spendingTranscribeDataset = Object.values(spendingTranscribeData);
			let delayed;
			...
			new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						...
						label: '{{ __('Total Estimated Voiceover Spending') }} (USD) ',
						data: spendingVoiceoverDataset,
						...
						label: '{{ __('Total Estimated Transcribe Spending') }} (USD) ',
						data: spendingTranscribeDataset,
			...
			var purchased_used_current_month_minutes = JSON.parse(`<?php echo $percentage['purchased_used_current_minutes']; ?>`);			
			var purchased_used_past_month_minutes = JSON.parse(`<?php echo $percentage['purchased_used_past_minutes']; ?>`);
			...
			(purchased_used_past_month[0]['data'] == null) ? purchased_used_past_month = 0 : purchased_used_past_month_minutes = purchased_used_past_month_minutes[0]['data'];
			...
			(purchased_past_month_minutes[0]['data'] == null) ? purchased_past_month_minutes = 0 : purchased_past_month = purchased_past_month_minutes[0]['data'];
			(purchased_used_current_month_minutes[0]['data'] == null) ? purchased_used_current_month_minutes = 0 : purchased_used_current_month_minutes = purchased_used_current_month_minutes[0]['data'];
			(purchased_used_past_month_minutes[0]['data'] == null) ? purchased_used_past_month_minutes = 0 : purchased_used_past_month_minutes = purchased_used_past_month_minutes[0]['data'];
			...
			var purchased_used_current_total_minutes = parseInt(purchased_used_current_month_minutes);	
			var purchased_used_past_total_minutes = parseInt(purchased_used_past_month_minutes);
			...
			var purchased_change_minutes = mainPercentageDifference(purchased_past_total_minutes, purchased_current_total_minutes);
			var purchased_used_change_minutes = mainPercentageDifference(purchased_used_past_total_minutes, purchased_used_current_total_minutes);
			...
			document.getElementById('purchased_minutes_difference').innerHTML = purchased_change_minutes;
			document.getElementById('purchased_minutes_used_difference').innerHTML = purchased_used_change_minutes;
