					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							success: function (data) {
								if (data == 'success') {
									Swal.fire('{{ __('Review Deleted') }}', '{{ __('Customer review has been successfully deleted') }}', 'success');	
									$("#faqsTable").DataTable().ajax.reload();								
								} else {
									Swal.fire('{{ __('Delete Failed') }}', '{{ __('There was an error while deleting this customer review') }}', 'error');
								}      
							},
							error: function(data) {
