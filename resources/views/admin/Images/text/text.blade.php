    ...
    <div class="page-header mt-5-7">
        <div class="page-leftheader">
            ...
            <ol class="breadcrumb mb-2">
                ...
                <li class="breadcrumb-item"><a href="{{route('admin.text.user',$folder->project_id)}}"><i
                            class="fa-solid fa-user-shield mr-2 fs-12"></i>{{ __('Text Users') }}</a></li>
                <li class="breadcrumb-item" aria-current="page"><a
                        href="{{route('admin.text.user.folder',['projectID' => $folder->project_id, 'userID' => $folder->id])}}"> {{ $folder->name }}</a></li>
...
@section('content')
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="card-header d-flex align-items-center">
                            <h3 class="card-title mb-0 mr-3">{{ __('All Text Results') }}</h3>
                ...
                <div class="card-body pt-2">
                    ...
                    <table id='userResultTable' class='table' width='100%'>
                        ...
                        <tr>
                            ...
                            <th width="12%">{{ __('Text') }}</th>
                            <th width="12%">{{ __('Translated Text') }}</th>
                            <th width="4%">{{ __('Folder Name') }}</th>
    ...
    <div id="sidebar" class="sidebar bg-light d-none" style="z-index: 9999;">
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div class="form-group mb-0">
                ...
                <select class="form-control custom-data" id="remark" name="remark">
                    ...
                    @foreach($remarks as $projectRemark)
        ...
        <div class=" mt-4" id="clickableDiv">
            ...
            <textarea class="form-control visibleTextarea" id="visibleTextarea" rows="3"
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div class="form-group mb-0">
                ...
                <select class="form-control custom-data" id="feedback">
                    <option value="" selected disabled>Select any feedback</option>
                ...
                <div class="modal-body p-0">
                    <div class="row no-gutters h-100">
                        ...
                        <div class="mt-8 text-center">
                            <!-- Added text area -->
                            <p class="translated_text form-control" id="translated_text"></p>
                        </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            var table = $('#userResultTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.text.user.folder.text',['id'=>$id]) }}",
                    data: function (d) {
            ...
            $(document).on('click', '.translated_text', function() {
                var text = $(this).text();
            ...
            // Convert input back to text when focus is lost
            $(document).on('blur', 'input', function() {
                var text = $(this).val();
                var p = $('<p class="translated_text form-control" id="translated_text">' + text + '</p>');
                $(this).replaceWith(p);
            });

            $('#textDataCarousel').on('slid.bs.carousel', function () {
                // Re-initialize click events on slide change
                $('.translated_text').click(function() {
                    var text = $(this).text();
                    var input = $('<input type="text" class="form-control" value="' + text + '"/>');
                    $(this).replaceWith(input);
                    input.focus();
                ...
                $('input').blur(function() {
                    var text = $(this).val();
                    var p = $('<p class="translated_text form-control">' + text + '</p>');
                    $(this).replaceWith(p);
                });
            });
            ...
            $(document).on('click', '.text-data-link', function (e) {
                ...
                for (var i = 0; i < textDataLinks.length; i++) {
                    ...
                    console.log(textDataLinks[i], 'textDataLinks[i]')
                    var textDataSrc = textDataLinks[i];

                    var carouselItem = '<div class="carousel-item' + activeClass + '" id="' + row.id + '">' +
                        '<div class="text-data-container d-block w-100">' + row.text + '</div>' +
                        '</div>';
                ...
                if (row.folder && row.folder.name) {
                    $('#folder').html(row.folder.name);
                } else {
                    $('#folder').html('N/A'); // Set a default value (e.g., 'N/A') if the folder name is not available
                }
                if (row.folder && row.folder.quality_assurance) {
                    $('#assigned_user').html(row.folder.quality_assurance.name);
                } else {
                ...
                $('#translated_text').html(row.translated_text);
                ...
                if (row.comment) {
                    ...
                    // Create a dummy div to decode the HTML entities
                    var dummyDiv = document.createElement("div");
                    dummyDiv.innerHTML = row.comment;

                    // Get the decoded value
                    var decodedComment = dummyDiv.innerText || dummyDiv.textContent;

                    console.log(decodedComment)
                    $('#hiddenTextarea').val(decodedComment);
                    $('#visibleTextarea').val(decodedComment);
                } else {
                    $('#visibleTextarea').val('');
                    $('#hiddenTextarea').val('');
                }
            ...
            $(document).on('click', '#nextButton, #prevButton', function (e) {
                ...
                $.ajax({
                    url: '/admin/user-next-text', // Update the URL to the actual endpoint that returns the text data
                    type: 'GET',
                    data: {
                    ...
                    success: function (response) {
                        ...
                        if (response.id) {
                            ...
                            $('#username').html(response.folder.user.name);
                            // $('#duration').html(formatDuration(response.length));
                            if (response.folder && response.folder.name) {
                                $('#folder').html(response.folder.name);
                            } else {
                                $('#folder').html('N/A'); // Set a default value (e.g., 'N/A') if the folder name is not available
                            }
                            if (response.folder && response.folder.quality_assurance) {
                                $('#assigned_user').html(response.folder.quality_assurance.name);
                            } else {
                                $('#assigned_user').html('N/A'); // Set a default value (e.g., 'N/A') if the folder name is not available
                            }

                            // Parse and format the 'created-on' date
                            var dateStr = response.created_at;
                            // Your date string
                            var date = new Date(dateStr); // Create a new Date object
                            ...
                            $('#created_at').html(formattedDate);                // Set audio source here

                            // Set audio source here
                            $('#translated_text').html(response.translated_text);
                            ...
                            if (response.comment) {
                                ...
                                var dummyDiv = document.createElement("div");
                                dummyDiv.innerHTML = response.comment;
            ...
            $(document).on('click', '#saveButton', function (e) {
                ...
                // Retrieve the edited text if the text is in an input
                var editedText;
                var activeInput = $('.carousel-item.active input');

                if (activeInput.length > 0) {
                    editedText = activeInput.val();
                } else {
                    editedText = $('.translated_text').text();
                }

                // Retrieve the comment, feedback and QA values
                var comment = $('#visibleTextarea').val();
                var feedback = $('#feedback').val();
                var remark = $('#remark').val();

                // Check if QA is required when feedback is 'incorrect'
                if (feedback === 'incorrect' && comment === '') {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        feedback: feedback,
                        remark: remark,
                        editedText: editedText,  // Include the edited text in the data
                    },
                    success: function (response) {
                        $("#userResultTable").DataTable().ajax.reload();
                        $("#nextButton").click();
