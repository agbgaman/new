...
{   
    ...
    public function upgrade(Request $request)
    {
        ...
        if ($current_version == $latest_version['latest_version']) {
            ...
            return redirect()->back()->with('success', __('You are already using the latest version '). $latest_version['latest_version']);
        }

        try {
            $response = $this->api->download_update($request->update_id, false, $request->version, $license = false, $client = false, $db_for_import = false);
        } catch(Exception $e) {
            return redirect()->back()->with('error', __('There was an error during software update. ') . $e->getMessage());
        }
        ...
        if ($response) {           
            return redirect()->back()->with('success', __('Software successfully was upgraded to version '). $request->version);
        } else {
            return redirect()->back()->with('error', __('Software was not updated correctly. Please try again or contact support'));
        }
