...
class LicenseController extends Controller
{
	...
	public function __construct(){ 
		$this->product_id = '433CD354';
		$this->api_url = 'https://license.berkine.space/';
		$this->api_key = '7D4F55C8098DE05B6A5C';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["6825dd7d09af19f"])){
				$_SESSION["6825dd7d09af19f"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["6825dd7d09af19f"])){
				$get_data = $this->call_api(
					'POST',
